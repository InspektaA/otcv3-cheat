...
const kv_config_link = "https://raw.githubusercontent.com/KadeDev/OnetapScripts/master/RainbowESP.js"; // Has to be a actuall link, like to a text file/raw file.
const kv_config_version = "1.1";
const kv_config_name = "RainbowESP";
...
function checkVersion()
{
    ...
    var request = http.request(options, function (res) {
        ...
        res.on('end', function () {
            Cheat.Print("[KV-AutoUpdate] Checking version...");
            if (!data.includes("const kv_config_version = " + '"' + kv_config_version + '"' + ";"))
            {
                Cheat.Print("[KV-AutoUpdate] Version out of date. Updating...")
                Cheat.PrintChat("[KV-AutoUpdate] Updating script, please reload your scripts...")
                fs.writeFile(kv_config_name + "_updated.js", data, function (err) {
                    if (err) throw err;
                    Cheat.Print("[KV-AutoUpdate] Update complete! Reload your scripts...")
                    return false;
                });
            }
        });
    ...
    request.on('error', function (e) {
        Cheat.Print("[KV-AutoUpdate] Failed to read the end url.")
    });
    request.end();
    return true;
}
