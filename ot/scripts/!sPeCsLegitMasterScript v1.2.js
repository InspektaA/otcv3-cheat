/*
           _____      _____ _      _                _ _     __  __            _               _____          _       _          __   ___
  	      | __ \    / ____ ( )    | |              (_) |   |  \/  |          | |            / ____ |        (_)     | |        /_ | |__ \ 
      ___ ||__) |__| |    | /___  | |     ___  __ _ _| |_  | \  / | __ _ ___ | | _ ___ _ __| (___   ___ _ __ _ _ __ | | _  __  _| |    ) |
   	 / __|  ___/ _ \ |      / __| | |    / _ \/ _` | | __| | |\/| |/ _` / __| __/ _ \ '__|  \___ \ / __| '__| | '_ \| __| \ \ / / |   / / 
     \__ \ |  |  __/ |____  \__ \ | |___|  __/ (_| | | |_  | |  | | (_| \__ \ ||  __/ |     ____) | (__| |  | | |_) | |_   \ V /| |_ / /_ 
...
function main() {
	...
	UI.AddLabel("________________________________________");
	UI.AddCheckbox("Trigger Ragebot");
	...
	Cheat.PrintColor([0, 255, 255, 255], "\n    - - - > sPeC's Legit Master Script v1.2 has been loaded! < - - -");
	Cheat.PrintColor([0, 255, 255, 255], "\n\n======================================================================\n");
	Cheat.PrintChat(" \x06sPeC's Legit Master Script has been loaded! \x0E[ \x06v1.2 \x0E] ");
}
...
function drawShit() {
	...
	if (getVal("Chat Functions")) {
		UI.SetEnabled("Auto Response", true);
		UI.SetEnabled("Clear Chat", true);
		UI.SetEnabled("Kill Say", true);
		UI.SetEnabled("Death Say", true);
		cChat = getVal("Clear Chat");
		kSay = getVal("Kill Say");
		dSay = getVal("Death Say");

		if (cChat == 1) {
			UI.SetEnabled("Ignore Team", true);
		} else if (cChat == 2) {
			UI.SetEnabled("Clear Chat Speeed", true);
		} else {
			UI.SetEnabled("Clear Chat Speed", false);
			UI.SetEnabled("Ignore Team", false);
		}

		if (kSay == 1) {
			UI.SetEnabled("Custom Kill Say", true);
		} else { UI.SetEnabled("Custom Kill Say", false); }

		if (dSay == 1) {
			UI.SetEnabled("Custom Death Say", true);
		} else { UI.SetEnabled("Custom Death Say", false); }

	} else {
		UI.SetEnabled("Auto Response", false);
		UI.SetEnabled("Ignore Team", false);
		UI.SetEnabled("Clear Chat", false);
		UI.SetEnabled("Kill Say", false);
		UI.SetEnabled("Death Say", false);
		UI.SetEnabled("Custom Kill Say", false);
		UI.SetEnabled("Custom Death Say", false);
	}

	if (getVal("Trigger Ragebot")) {
		UI.SetEnabled("Avoid Auto Wall", true);
		oAWG = UI.GetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)");
		if (UI.GetValue("Rage", "PISTOL", "Pistol config", "Override default")) {
			rP = true;
			oAWP = UI.GetValue("Rage", "PISTOL", "Damage", "Minimum damage (behind wall)");
		} else { oAWP = UI.GetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)"); rP = false; }
		if (UI.GetValue("Rage", "HEAVY PISTOL", "Heavy pistol config", "Override default")) {
			rHP = true;
			oAWHP = UI.GetValue("Rage", "HEAVY PISTOL", "Damage", "Minimum damage (behind wall)");
		} else { oAWHP = UI.GetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)"); rHP = false; }
		if (UI.GetValue("Rage", "SCOUT", "Scout config", "Override default")) {
			rS = true;
			oAWS = UI.GetValue("Rage", "SCOUT", "Damage", "Minimum damage (behind wall)");
		} else { oAWS = UI.GetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)"); rS = false; }
		if (UI.GetValue("Rage", "AWP", "AWP config", "Override default")) {
			rA = true;
			oAWA = UI.GetValue("Rage", "AWP", "Damage", "Minimum damage (behind wall)");
		} else { oAWA = UI.GetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)"); rA = false; }
		if (UI.GetValue("Rage", "AUTOSNIPER", "Auto config", "Override default")) {
			rAS = true;
			oAWAuto = UI.GetValue("Rage", "AUTOSNIPER", "Damage", "Minimum damage (behind wall)");
		} else { oAWAuto = UI.GetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)"); rAS = false; }

		if (getVal("Avoid Auto Wall")) {
			localEnt = Entity.GetLocalPlayer();
			localHead = Entity.GetHitboxPosition(localEnt, 0);
			enemies = Entity.GetEnemies();
			UI.SetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)", 100);
			UI.SetValue("Rage", "PISTOL", "Damage", "Minimum damage (behind wall)", 150);
			UI.SetValue("Rage", "HEAVY PISTOL", "Damage", "Minimum damage (behind wall)", 150);
			UI.SetValue("Rage", "SCOUT", "Damage", "Minimum damage (behind wall)", 150);
			UI.SetValue("Rage", "AWP", "Damage", "Minimum damage (behind wall)", 150);
			UI.SetValue("Rage", "AUTOSNIPER", "Damage", "Minimum damage (behind wall)", 150);
			for (i = 0; i < enemies.length; i++) {
				headHitbox = Entity.GetHitboxPosition(enemies[i], 0);
				lFootHitbox = Entity.GetHitboxPosition(enemies[i], 11);
				rFootHitbox = Entity.GetHitboxPosition(enemies[i], 12);
			...
			UI.SetValue("Rage", "PISTOL", "Damage", "Minimum damage (behind wall)", oAWP);
			UI.SetValue("Rage", "HEAVY PISTOL", "Damage", "Minimum damage (behind wall)", oAWHP);
			UI.SetValue("Rage", "SCOUT", "Damage", "Minimum damage (behind wall)", oAWS);
			UI.SetValue("Rage", "AWP", "Damage", "Minimum damage (behind wall)", oAWA);
			UI.SetValue("Rage", "AUTOSNIPER", "Damage", "Minimum damage (behind wall)", oAWAuto);
			UI.SetValue("Rage", "GENERAL", "Accuracy", "Minimum damage (behind wall)", oAWG);
		}
		
		if (getVal("Avoid Auto Wall") && visiblePeeking && isTrigger) {
			UI.SetValue("Legit", "General", "Enabled", 0);
			UI.SetValue("Rage", "GENERAL", "Enabled", 1);
		} else if (!getVal("Avoid Auto Wall") && visPeeking && isTrigger) {
			UI.SetValue("Legit", "General", "Enabled", 0);
			UI.SetValue("Rage", "GENERAL", "Enabled", 1);
		} else {
	...
	if (getVal("Anti-Flicker")) {
		...
		if (fps <= tickRate) { UI.SetValue("Anti-Aim", "Legit Anti-Aim", "Enabled", 0); } else { UI.SetValue("Anti-Aim", "Legit Anti-Aim", "Enabled", 1); }
		if (ping >= maxPing) { UI.SetValue("Anti-Aim", "Legit Anti-Aim", "Enabled", 0); } else { UI.SetValue("Anti-Aim", "Legit Anti-Aim", "Enabled", 1); }
	} else {
		UI.SetEnabled("Max Ping", false);
	}
